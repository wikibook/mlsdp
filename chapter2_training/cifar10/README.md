...
MLFlow 를 사용한 Cifar10 데이터셋의 학습 파이프라인입니다.
...
이 프로그램에서는 conda 커맨드를 사용하기 위해, Anaconda/miniconda 를 설치합니다.
Anaconda/miniconda 실행환경은 pyenv 등의 가상환경으로 준비할 것을 권장합니다.
pyenv 로 환경을 구축하는 방법은 다음과 같습니다.

```sh
# 2021/02 기준 최신 anaconda 환경을 선택
...
# pyenv로 가상환경에 anaconda3-2020.02 선택
$ pyenv local anaconda3-5.3.1

# 가상환경이 anaconda3-2020.02 임을 확인
$ pyenv versions
  system
* anaconda3-5.3.1
...
이 프로그램은 `/tmp/ml-system-in-actions/chapter2_training/cifar10/` 디렉토리에서 실행할 것을 상정해 작성되었습니다.
레포지토리를 `/tmp/` 디렉토리에 클론한 뒤 실행하거나, 다른 디렉토리에 있는 경우는 `/tmp/ml-system-in-actions/chapter2_training/cifar10/` 로 심볼릭 링크해서 실행해 주십시오.
...
#     -f Dockerfile .
# 출력 생략
# docker 이미지로 shibui/ml-system-in-actions:training_pattern_cifar10_0.0.1 이 빌드됩니다.
```
...
학습이 완료할 때 까지, 수분에서 수십분 정도가 걸리는 경우가 있습니다.

실행로그（일부）는 다음과 같습니다. 

<details><summary>로그</summary><div>
